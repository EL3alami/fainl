...
function ClassScheduleContent({ studentSession }) {
  ...
  const fetchMySchedule = async () => {
    try {
      setLoading(true);
      const data = await registrationApi.getMyCourses(student.id);

      // Filter out enrollments that don't have schedule info yet
      const mapped = data
        .filter(e => e.schedule_info)
        .map(e => ({
          id: e.id,
          course: e.course?.name_en,
          code: e.course?.code_en,
          day: e.schedule_info.day,
          time: `${e.schedule_info.start_time} - ${e.schedule_info.end_time}`,
          room: e.schedule_info.room,
          professor: e.schedule_info.professor?.name_en || "TBA"
        }));
    ...
    <div className="admin-page animate-in">
      <div className="page-header">
        <div className="title-section">
          ...
          <p className="breadcrumb">{student?.name_en} · Active Semester Schedule</p>
        </div>
      </div>

      {loading ? (
        <div style={{ textAlign: 'center', padding: '40px' }}>Loading your classes...</div>
      ) : schedule.length === 0 ? (
        <div className="empty-state card-style">
          <div style={{ fontSize: '40px', marginBottom: '15px' }}>📅</div>
          <h3>No Classes Scheduled</h3>
          <p>We couldn't find any scheduled classes for your registered courses yet. Please check back later or contact the academic office.</p>
        </div>
      ) : (
        <div className="data-section">
          ...
          <h2 style={{ marginBottom: '20px', fontSize: '20px' }}>Daily View</h2>
            ...
            {days.map((day) => {
              ...
              return (
                <div key={day} className="card-style" style={{ padding: "20px", borderTop: dayClasses.length > 0 ? "4px solid #4f46e5" : "1px solid #e2e8f0" }}>
                  ...
                  {dayClasses.length === 0 ? (
                    <p style={{ color: "#94a3b8", fontSize: "14px", fontStyle: 'italic' }}>No classes today</p>
                  ) : (
...
export default withStudentVerification(ClassScheduleContent);
